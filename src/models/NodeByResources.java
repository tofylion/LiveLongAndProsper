...
public class NodeByResources implements Comparator<Node> {
    ...
    public NodeByResources(boolean isTieBreaking, LLAPSearch problem){
        this.isTieBreaking = isTieBreaking;
        resourcesPerBuild1 = getSlice(problem.getBuildOneInfo(), 1, 5);
        resourcesPerBuild2 = getSlice(problem.getBuildTwoInfo(), 1, 5);
        pricesPerBuild = problem.getUnitPrices();
    ...
    public int compare(Node n1, Node n2) {
        if(getCost(n1) > getCost(n2)){
            ...
            if (isTieBreaking) {
                ...
                Comparator<Node> TieBreakingHeuristic = new NodeByBuild(true, problem);
